...
from data.word2vec import getEmbedding
...
def save_model(model, name, reader, option, **variables):
    ...
    global_step = option["global_step"]
    name = name + "." + str(global_step)
...
def parseargs_train(args):
    ...
    msg = "word embeddings obtained from word2vec"
    parser.add_argument("--word2vec", nargs=2, help=msg)
    ...
    parser.add_argument("--filter", type=int, help=msg)
    
    msg = "number of layers of both encoder and decoder"
    parser.add_argument("--num_layers", type=int, help=msg)
    msg = "number of heads of multihead attention"
    parser.add_argument("--num_heads", type=int, help=msg)

    parser.add_argument("--attention_dropout", type=float, help=msg)
    parser.add_argument("--residual_dropout", type=float, help=msg)
    parser.add_argument("--relu_dropout", type=float, help=msg)

    msg = "warmup steps, default 10000"
    parser.add_argument("--warmup", type=int, help=msg)
...
def parseargs_decode(args):
    ...
    msg = "trained model"
    parser.add_argument("--model", required=True, help=msg)
    msg = "source corpus, which need to be translated"
    parser.add_argument("--corpus", required=True, help=msg)
    msg = "translation corpus"
    parser.add_argument("--translation", required=True, help=msg)
    msg = "beam size"
    parser.add_argument("--beamsize", default=5, type=int, help=msg)
    msg = "normalize probability by the length of cadidate sentences"
    parser.add_argument("--normalize",  default=True, type=bool, help=msg)
...
def override(option, newopt):
    ...
    # word2vec
    if newopt["word2vec"] is None and option["word2vec"] is None:
        raise RuntimeError("error: no word2vecs specified")
    ...
    if option["word2vec"] is None:
        option["word2vec"] = newopt["word2vec"]
        source_word2vec = getEmbedding(option["word2vec"][0])
        target_word2vec = getEmbedding(option["word2vec"][1])
        option["word2vecs"] = [source_word2vec, target_word2vec]

    # model parameters
    override_if_not_none(option, newopt, "embedding")
    override_if_not_none(option, newopt, "hidden")
    override_if_not_none(option, newopt, "filter")

    # dropouts
    override_if_not_none(option, newopt, "attention_dropout")
    override_if_not_none(option, newopt, "residual_dropout")
    override_if_not_none(option, newopt, "relu_dropout")

    override_if_not_none(option, newopt, "num_layers")
    override_if_not_none(option, newopt, "num_heads")

    # training options
    override_if_not_none(option, newopt, "maxepoch")
    override_if_not_none(option, newopt, "alpha")
    override_if_not_none(option, newopt, "warmup")
...
def print_option(option):
    ...
    print "filter:", option["attention_dropout"]
    print "filter:", option["residual_dropout"]
    print "filter:", option["relu_dropout"]

    print "filter:", option["num_layers"]
    print "filter:", option["num_heads"]

    print "maxepoch:", option["maxepoch"]
    print "alpha:", option["alpha"]
    print "warmup:", option["warmup"]
...
def train(args):
    ...
    model = NMT(option["num_layers"], option["num_heads"], option["attention_dropout"], 
                option["residual_dropout"], option["relu_dropout"], option["embedding"], 
                option["hidden"], option["filter"], len(isvocab), len(itvocab), 
                source_word2vec, target_word2vec, initializer=initializer)
    ...
    with tf.Session(config=config):
	    ...
	    alpha = (1/float(option["embedding"])**0.5) * min(1/float(global_step)**0.5, global_step/float(option["warmup"])**1.5)
...
def load_average_model(names):
	...
	fd = open(name, "r")
    	option = cPickle.load(fd)
    	var_names = cPickle.load(fd)
    	var_vals = dict(np.load(fd))
	...
	if "indices" in var_vals:
            option["indices"] = vals["indices"]
	fd.close()
    
    params = [(n, vals[n]) for n in var_names]
